...
public class MapFile {
    public static void main(String[] argv) throws Exception {
        ...
        File tempFile = new File("mmaptest.txt");
        ...
        cow.force(); // 在写时拷贝缓冲区上调用force不会有任何作用，mode.read, mode.private将不会对磁盘文件进行更改
        System.out.println("Change to COW buffer");
        showBuffers(ro, rw, cow); // 写时拷贝，不会影响其他buffer
        ...
        showBuffers(ro, rw, cow); // 其他buffer的写入对写时复制buffer不可见
        ...
        ro = channel.map(FileChannel.MapMode.READ_ONLY, 0, channel.size());
        showBuffers(ro, rw, cow);
